...
@router.get("/{assembly_name}", response_description="Get brothers from assembly id", response_model=List[Brother])
def find_brother_of_assembly(assembly_name: str, request: Request):
    if (brother := request.app.database["brothers"].find({"assembly": "Bezons"})) is not None:
        return brother
    raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Brothers of assembly {assembly_name} not found")
